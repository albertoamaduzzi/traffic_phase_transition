    ...
    if len(x)==1:
        ...
        for source_vertex in planar_graph.graph.vertices():
            for target_vertex in planar_graph.graph.vertices(): 
                add_edge2graph(planar_graph,source_vertex,target_vertex,target_vertex,False)
        ...
        print([v for v in planar_graph.graph.vertices()])
...
def add_centers2graph(planar_graph,r0,number_nodes,side_city,number_initial_nodes = 1):
    '''
...
#        initialize_distance_matrix(planar_graph,x,y)
        add_initial_points2graph(planar_graph,r0,number_initial_nodes,side_city)
#        for point_idx in range(len(x)):
#            add_vertex(planar_graph)
#            id_ = get_id_last_vertex(planar_graph)
#            vertex = get_last_vertex(planar_graph)
...
def evolve_uniquely_attracted_vertex(planar_graph,growing_node,available_vertices):
    '''
        Evolve street from (in graph) points that have a unique attracting vertex 
    '''
    if growing_node not in planar_graph.end_points:
        intersection = True
    else:
        intersection = False
...
#    print('available attracting vertices: ',[planar_graph.graph.vp['id'][v] for v in planar_graph.graph.vp['relative_neighbors'][growing_node]])
    ...
    print('uniquely available vertex: ',available_vertices)
    ...
    print('length segment: ',np.sqrt(dx**2+dy**2))
    ...
    add_edge2graph(planar_graph,planar_graph.graph.vertex(growing_node),added_vertex,vertex_relative_neighbor,intersection)
    ## Find the road where the points belongs to (looking at the starting vertex it is generated from)
    plot_relative_neighbors(planar_graph,growing_node,added_vertex,available_vertices)
...
def evolve_multiply_attracted_vertices(planar_graph,growing_node,available_vertices):
    '''
        NOTE:
...
#    print('available attracting vertices: ',[planar_graph.graph.vp['id'][v] for v in available_vertices])
    ...
    if np.sqrt(dx**2+dy**2)>epsilon: 
        ...
        update_distance_matrix(planar_graph,old_points,new_point)
        added_vertex = add_point2graph(planar_graph,growing_node,dx,dy)
        add_edge2graph(planar_graph,planar_graph.graph.vertex(growing_node),added_vertex,available_vertices,intersection)
        plot_relative_neighbors(planar_graph,growing_node,added_vertex,available_vertices)                            
...
#                            vertex_relative_neighbor = planar_graph.graph.vertex(int_idx_relative_neighbor)
            ...
            update_distance_matrix(planar_graph,old_points,new_point)
            added_vertex = add_point2graph(planar_graph,growing_node,dx,dy)
            add_edge2graph(planar_graph,planar_graph.graph.vertex(growing_node),added_vertex,intersection)  
            plot_relative_neighbors(planar_graph,growing_node,added_vertex,available_vertices)
...
#        print('attracted vertices: ',[planar_graph.graph.vp['id'][relative_neighbor] for relative_neighbor in planar_graph.graph.vp['relative_neighbors'][attracting_node]])
    ...
    for growing_node in planar_graph.end_points:
        ...
        print('available vertices node: {}'.format(growing_node),available_vertices)
...
#                print('growing node: ',planar_graph.graph.vp['id'][growing_node],' coords: ',planar_graph.graph.vp['pos'][growing_node])
        ...
        if number_relative_neighbors_growing_node==1:
            evolve_uniquely_attracted_vertex(planar_graph,growing_node,available_vertices)
        elif number_relative_neighbors_growing_node>1: # a relative neighbor is also a neighbor -> kill the growth
            evolve_multiply_attracted_vertices(planar_graph,growing_node,available_vertices)
...
#        print('attracted vertices: ',[planar_graph.graph.vp['id'][relative_neighbor]for relative_neighbor in planar_graph.graph.vp['relative_neighbors'][attracting_node]])
        ...
        for growing_node in planar_graph.graph.vp['relative_neighbors'][attracting_node]:
            if growing_node not in already_grown_vertices and planar_graph.graph.vp['is_in_graph'][growing_node]:
                available_vertices = planar_graph.graph.vp['relative_neighbors'][growing_node]
                print('available vertices node: {}'.format(growing_node),available_vertices)
                number_relative_neighbors_growing_node = len(planar_graph.graph.vp['relative_neighbors'][growing_node])
#                print('growing node: ',planar_graph.graph.vp['id'][growing_node],' coords: ',planar_graph.graph.vp['pos'][growing_node])
